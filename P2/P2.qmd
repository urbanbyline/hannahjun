...
-   Estimating vehicle availability model using data using similar predictors to those in the the TDM23 vehicle availability submodel and data from the 2017 National Household Travel Survey,
...
-   WORKER: Whether the person is a worker.
    ...
    -   QUESTION: Binary is between 1 and 2. 1 is YES. 2 is NO.

-   DRIVER: Whether the person is a driver.

    -   QUESTION: Binary is between 1 and 2. 1 is YES. 2 is NO.

-   HOME OWN IS 1 and RENT IS 2
...
QUESTION: Is this basically unemployed, seniors, and kids who can drive?
...
person_summary <- person_data |> 
  group_by(HOUSEID) |> 
  summarize(HOMEOWN = first(HOMEOWN), .groups = "drop")  # Or use mode()

hh_data <- hh_data |> 
  left_join(person_summary, by = "HOUSEID")

hh_data <- hh_data |> 
  mutate(HOMEOWN = case_when(
    HOMEOWN == "01" ~ "Own",
    HOMEOWN == "02" ~ "Rent",
    TRUE ~ HOMEOWN  # Keeps any other values unchanged
  ))
...
Is the way to read then the n_child compared to the n_seniors - is having one more child in a household vs having one more senior the vehicle insufficiency is higher

*Question:*

What does -8.37 utility mean in comparison to 0.24. Is the range 0-100?
